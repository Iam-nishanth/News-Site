...
export const DraftPostsContext = createContext<
  DraftPostsContextValue | undefined
>(undefined);
...
export const DraftPostsProvider: React.FC<DraftPostsProviderProps> = ({
  children,
}) => {
  const [draftPosts, setDraftPosts] = useState<Post[]>(() => {
    if (typeof window !== "undefined") {
      // Check if the code is running in the browser
      const storedDraftPosts = JSON.parse(
        localStorage.getItem("draftPosts") || "[]"
      );
  ...
  const publishPost = (postId: string) => {
    ...
    setDraftPosts((prevDraftPosts) =>
      prevDraftPosts.filter((post) => post.title !== postId)
    );
  };
  ...
  return (
    <DraftPostsContext.Provider
      value={{ draftPosts, addDraftPost, publishPost, open }}
    >
      {children}
    </DraftPostsContext.Provider>
  );
