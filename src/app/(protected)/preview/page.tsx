...
export default async function Preview() {
  ...
  const news = Array.from({ length: 6 }, () => draftPosts).flat();
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {news.map((post, index) => {
            return (
              <div
                key={index}
                className=" w-full flex flex-col gap-5 justify-between p-4 border border-gray-700 dark:border-muted rounded-lg"
              >
                <h1>{post?.title}</h1>
                <p className="w-full flex gap-3">
                  Category:{" "}
                  <span className="w-16 flex justify-center items-center bg-green-800 font-Roboto text-sm tracking-wide uppercase">
                    {post?.categorySlug}
                  </span>
                </p>
                <div>
                  ...
                  <p>
                    Updated:{" "}
                    <span className=" text-blue-400">
                      {post && formatCreatedAt(post.updatedAt)}
                    </span>
                  </p>
                </div>
                <Link
