...
const SigninPage = async ({ searchParams }: SignInProps) => {
  ...
  //   return <h1>{searchParams.error}</h1>;
  // }
  ...
  return (
    <MaxWidthWrapper className="flex justify-center items-center h-full">
      <div className="w-full hidden sm:flex lg:flex-1 sticky top-0 h-auto lg:h-[73vh] justify-center items-center flex-col">
        <h1 className="text-3xl font-semibold underline decoration-cyan-500 underline-offset-4 -mb-6 font-Gentium">
      ...
      <div className="w-full max-w-[500px] pt-5 px-2 sm:px-0 sm:pt-0 ">
        <h1 className="block sm:hidden text-2xl font-semibold underline decoration-cyan-500 underline-offset-4 mb-4 text-center font-Gentium">
          Sign in to your account
        </h1>
        <SignInForm
          callbackURL={searchParams?.callbackUrl}
          callbackError={searchParams.error && searchParams.error}
        />
      </div>
    </MaxWidthWrapper>
  );
