...
export default function EditorSection({ categories }: Props) {
  ...
  const editor = useCreateBlockNote({
    initialContent: [
      ...
      {
        type: "paragraph",
        content: "Press the '/' key - to see the editing options",
      },
      {
        type: "paragraph",
        content: "Please delete before typing the article",
      },
    ],
  ...
  // const OnChange = async () => {
  //   const html = editor.domElement.outerHTML;

  //   // Assuming `editor.domElement` is the root element of the editor's content
  //   const computedStyles = window.getComputedStyle(editor.domElement);

  //   // This is a simplified example. You'll need to iterate over the computed styles
  //   // and extract the relevant CSS rules.
  //   const relevantStyles = Object.entries(computedStyles)
      ...
      >
        ...
        <div className="space-y-2">
          <Label
            className={cn(
              "pl-2 text-base font-medium",
              errors?.categorySlug && "text-red-600"
            )}
          >
        ...
        <div className="space-y-2">
          ...
          {errors?.imgCaption && (
            <p className="text-red-500">{errors.imgCaption.message}</p>
          )}
        </div>
        <div className="space-y-2">
          <Label
          ...
          <AlertDialog>
            <AlertDialogTrigger asChild>
              <LoadingButton
                variant="outline"
                disabled={!preview}
                className="uppercase font-semibold tracking-wider w-36"
              >
                Preview
              </LoadingButton>
            </AlertDialogTrigger>
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  On clicking continue you will be redirected drafts page. Make
                  sure you the post is saved.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
