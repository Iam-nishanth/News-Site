...
const CategoryCard = ({ className, Content }: Props) => {
    const { categorySlug, id, title, content, featuredImg, user, createdAt, slug } = Content;
        ...
        <div className={cn('w-full h-full border border-[#E0DEDE] dark:border-[#e0dede5b] dark:text-gray-300', className)}>
                ...
                {!featuredImg ? (
                    <div>
                        <div className="ml-3 mt-3 w-fit h-5 bg-blue-700 z-10 flex justify-center items-center">
                            <span className="text-sm uppercase text-white tracking-wide px-2 py-1">{categorySlug}</span>
                        </div>
                    </div>
                ) : (
                    <div className="w-full min-h-[230px] relative overflow-hidden group">
                        ...
                        <Image src={featuredImg} alt={title} layout="fill" className="object-cover dark:brightness-[0.8] group-hover:scale-110 transition-transform duration-500" />
                    </div>
                )}
            </Link>
            <div className="p-4 flex flex-col gap-4">
                <p className="flex gap-2 text-black dark:text-gray-500 items-center">
                    ...
                    <span className="text-sm font-medium uppercase tracking-wide">{DDMMYYYY(createdAt)}</span>
                </p>
                <div className="flex flex-col gap-1">
                    <Link href={`/${categorySlug}/${id}`} className="text-black dark:text-gray-300 hover:heading-underline">
